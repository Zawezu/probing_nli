...
def generate_activations(
    layer_number, model_name, save_to_disk=True, amount_to_generate=None
):
    all_acts = []
    all_labels = []
    ...
    try:
        ...
        for (sentence_a, sentence_b), label in tqdm(
            sick_en_dataloader_train, desc=f"Probing Layer {layer_number}"
        ):
...
def generate_activations_all_layers(
    model_name, save_to_disk=True, amount_to_generate=None
):
    ...
    try:
        for (sentence_a, sentence_b), label in tqdm(
            sick_en_dataloader_train, desc="Extracting all layers"
        ):
            prompt = f"Premise: {sentence_a} Hypothesis: {sentence_b} Label:"
            tokens = tokenizer(prompt, return_tensors="pt").to(device)
...
if __name__ == "__main__":
    sick_en_dataset_train, sick_en_dataloader_train = get_dataset_and_dataloader(
        "en", "train"
    )
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(
        olmo_filepath, local_files_only=True
    ).to(device)
